...
class GraphExpander {
    ...
    public void expand() {
        LinkedList<Integer>[] expandedGraph = new LinkedList[graphList.length];
    ...
    public String toString() {
        ...
        for(int j=0;j<graphList.length;j++) {
            LinkedList<Integer> vertex = graphList[j];
            if(vertex.size() < 1) graphString.append("\n");
            for(int edge: vertex) {
                graphString.append(edge);
                graphString.append(" ");
            }
            graphString.deleteCharAt(graphString.length() - 1);
            graphString.append("\n");
        }
