...
public class ST<Key extends Comparable<Key>, Value> implements Iterable<Key> {
    ...
    public Key ceiling(Key key) {
        if (isEmpty()) throw new NoSuchElementException();
        ST<Key, Value> additional = new ST<>();
    ...
    Iterable<Key> keys(Key lo, Key hi) {
        return new KeyIteratorLowHigh(lo, hi);
    }

    private class KeyIteratorLowHigh implements Iterator<Key>, Iterable<Key> {

        private Node current;
        private Node last;

        KeyIteratorLowHigh(Key lo, Key hi) {
            last.key = hi;
            last.value = get(hi);
            current.key = lo;
